...
//        window.plugins.sqlDB.copy("sonna.sqlite", copySuccess, copyError);
//        db = window.openDatabase("sqlite", "1.0", "sqlite", 20000000);
//        db.transaction(
//            function (tx) {
//                createTable(tx);
//                addSampleData(tx);
...
OpenMyDatabase = function() {
    ...
    if(typeof mac_browser_test === 'undefined'){
        ...
        db = window.sqlitePlugin.openDatabase({name:"sonna.sqlite"});
...
//
////        display("g2b1", "0").done(function (page) {
//
//            $.cookie("book_code", page.book_code);
//            $.cookie("page_id", page.page_id);
//
...
function createTable (tx) {
 ...
 function addSampleData (tx, employees) {
...
function display (book_code, page_id) {
    var deferred = $.Deferred();
